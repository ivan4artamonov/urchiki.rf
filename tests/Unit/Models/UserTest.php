...
test('normalizeEmail обрезает пробелы и приводит к нижнему регистру', function (): void {
    expect(User::normalizeEmail('  Test@Example.COM  '))->toBe('test@example.com');
});

test('whereEmailNormalized находит пользователя без учёта регистра', function (): void {
    User::factory()->create(['email' => 'find@example.com']);

    $found = User::query()->whereEmailNormalized('FIND@Example.COM')->first();
